...
opcode_specs: dict[int, Callable[..., None]] = {}


def opcode(num: int) -> Callable[[Callable[..., None]], Callable[..., None]]:
    """Decorator for registering opcodes"""

    def decorator(func: Callable[..., None]) -> Callable[..., None]:
        if num in opcode_specs:
            raise ValueError(
                f'Duplicate opcode {num}: {func.__name__} vs {opcode_specs[num].__name__}'
            )
...
class Machine:
    ...
    def load_program(self, filename: str = 'challenge.bin') -> None:
        ...
        self.m[: len(data_array)] = data_array
    ...
    def save_state(self, signum: int, frame: Any) -> None:
        ...
        with open(f'saves/{timestamp}', 'wb') as f:
            pickle.dump(self, f)
        logging.info(f'SAVED STATE. TIMESTAMP: {timestamp}')

    def run(self) -> None:
        while True:
            logging.debug(f'\nREGISTERS: {" ".join(str(i) for i in self.r)}')
